...
/**
 * Provides methods for working with {@link org.kuali.rice.krad.maintenance.MaintenanceDocument}.
 *
...
public interface MaintenanceDocumentService {
	 ...
	 *
	 * <p>This includes retrieving the data object for edit or copy, clearing fields</p>
	 *
     ...
     *
     * <p>For edit and copy actions, the old record is retrieved and prepared for
     ...
     * <code>Maintainble</code> implementations are invoked.</p>
     *
     ...
     */
    void setupMaintenanceObject(MaintenanceDocument document, String maintenanceAction,
	 ...
	 * maintenance record.
	 *
	 * <p>If any are pending and locked, thereby blocking this document, then the
	 * docHeaderId/documentNumber of the blocking locked document is returned.
	 * Otherwise, if nothing is blocking, then null is returned.</p>
	 *
	 ...
	 *
	 * <p>This method deletes the locks for the given document number. It is called
	 * when the document is final, thus it can be unlocked, or when the locks
	 * need to be regenerated (thus they get cleared first).</p>
	 *
	 ...
	 * the Dao, but the caller doesn't.
	 *
	 * <p>This method stores the given list of maintenance locks. Typically these
	 * will all be for the same document.</p>
	 *

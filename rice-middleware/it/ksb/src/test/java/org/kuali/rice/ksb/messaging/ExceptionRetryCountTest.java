...
public class ExceptionRetryCountTest extends KSBTestCase {
    ...
    public void testRetryCount() throws Exception {
        ...
        KSBJavaService explodingQueue = KsbApiServiceLocator.getMessageHelper().getServiceAsynchronously(
        ...
        while (i++ < 30) {
            ...
            if (queuedItems.size() != 1) {
                fail("test setup wrong should have a single item in the queue but instead had "
                        + queuedItems.size()
                        + ".");
            }
        ...
        assertEquals("Service should have been called 3 times", 3, TestHarnessExplodingQueue.NUM_CALLS);
        ...
        assertEquals("Message should be in exception status", KSBConstants.ROUTE_QUEUE_EXCEPTION,
                message.getQueueStatus());

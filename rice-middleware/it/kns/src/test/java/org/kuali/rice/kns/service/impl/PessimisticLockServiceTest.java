...
import org.kuali.rice.krad.test.document.AccountType2MaintainableImpl;
...
public class PessimisticLockServiceTest extends KNSTestCase {
    ...
    /**
     * tests the PessimisticLockService's ability to establish pessimistic locks for maintenance documents (via
     * maintainables) that
    ...
    public void testPessimisticLockingWithCustomMaintainableLockDescriptors() throws Exception {
        MaintenanceDocument maintDoc = (MaintenanceDocument) KRADServiceLocatorWeb.getDocumentService().getNewDocument(
                "AccountType2MaintenanceDocument");
        assertTrue("The AccountType2MaintenanceDocument should be using pessimistic locking",
                KRADServiceLocatorWeb.getDataDictionaryService().getDataDictionary().getDocumentEntry(
                        maintDoc.getNewMaintainableObject().getDataObjectClass().getSimpleName()
                                + "MaintenanceDocument").getUsePessimisticLocking());
        assertTrue("The AccountType2MaintenanceDocument should be using custom lock descriptors",
                maintDoc.useCustomLockDescriptors());
        ...
        assertTrue("The AccountType2MaintainableImpl should be using custom lock descriptors",
                newMaint.useCustomLockDescriptors());
    ...
    /**
     * A convenience method for testing the custom lock descriptors of documents (and on the maintainables of
     * maintenance documents).
     *
     * @param testDoc The document to test pessimistic locking on (or the maintenance document with maintainables to
     * test on).
     ...
     * @param LOCK_VALUE2 Another possible object to store in a UserSession for generating lock descriptors on the
     * testDoc.
     * @throws Exception
     */
    private void assertCustomLockDescriptorsAreWorking(Document testDoc, final String LOCK_KEY,
            final Serializable LOCK_VALUE1, final Serializable LOCK_VALUE2) throws Exception {
        ...
        assertNotSame("'quickstart' and 'admin' should have different custom lock descriptors now", allDescriptors[0],
                allDescriptors[1]);
        finalModes = lockService.establishLocks(testDoc, editMode, adminSession.getPerson());
        ...
        testDoc.refreshPessimisticLocks();
        assertTrue("There should not be any pessimistic locks present on the document",
                testDoc.getPessimisticLocks().isEmpty());
    }

 ...
 */
public class FreemarkerAftTmplMthdBlanketAppGenerator {
    ...
    //Configuration
    private static TemplateLoader templateLoader = new ClassTemplateLoader(FreemarkerAftTmplMthdBlanketAppGenerator.class, DIR_TMPL);
    
    public static void main(String[] args) throws Exception {
        cfg.setTemplateLoader(templateLoader);

        String propLocation = "/GenFiles/AdminTmplMthdBlanketAppAftNavBase.properties";
        if (args.length > 0) {
    ...
    private static void createFile(String propLocation, String template) throws Exception {
        try {
            InputStream in = FreemarkerAftTmplMthdBlanketAppGenerator.class.getResourceAsStream(propLocation);

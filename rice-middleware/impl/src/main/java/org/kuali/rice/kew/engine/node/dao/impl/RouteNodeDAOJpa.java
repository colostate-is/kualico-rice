...
public class RouteNodeDAOJpa implements RouteNodeDAO {
    ...
    private static final String CURRENT_ROUTE_NODE_NAMES_SQL = "SELECT rn.nm, rn.typ" +
    ...
    public List<String> getActiveSimpleRouteNodeNames(final String documentId) {
        ...
        List<String> names = template.execute(
                new PreparedStatementCreator() {
                    public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
                        PreparedStatement statement = connection.prepareStatement(
                                "SELECT rn.nm, rn.typ FROM krew_rte_node_t rn, krew_rte_node_instn_t rni WHERE rn.rte_node_id "
                                        + "= rni.rte_node_id AND rni.doc_hdr_id = ? AND rni.actv_ind = ?");
                        return statement;
                    }
                },
    ...
    /**
     * This method is a bit of a hack because in order to check if a node is "simple" or not, we actually
     * need to instantiate it's type and check the java class type. RouteHelper can do this for us but it
     * needs an actual RouteNode object to do this work. To avoid the full cost of loading the full
     * RouteNode object and it's associated DocumentType, we will mock out what we need here to get the job
     * done.
     */
    private boolean isSimpleNode(String type) {
        // construct a "mock" RouteNode so that we can fool the RouteHelper
        RouteNode routeNode = new RouteNode();
        routeNode.setNodeType(type);
        // document type must be non-null since route helper will attempt to get the application id off of it
        // though it's ok if the application id is null
